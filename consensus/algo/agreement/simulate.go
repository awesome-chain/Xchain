...
func (i *instant) TimeoutAt(d time.Duration) <-chan time.Time {
	...
	if d == FilterTimeout() && !i.HasPending("pseudonode") {
		close(ta)
	}
	return ta
}
...
func Simulate(dbname string, n basics.Round, roundDeadline time.Duration, ledger Ledger, keyManager KeyManager, proposalFactory BlockFactory, proposalValidator BlockValidator, log logging.Logger) error {
	...
	service := MakeService(parameters, vrfKey)
	service.Start()
