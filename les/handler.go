...
import (
	...
	"github.com/awesome-chain/Xchain/core/rawdb"
	...
	"github.com/awesome-chain/Xchain/light"
	...
	"github.com/awesome-chain/Xchain/trie"
)
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light TTC protocol")
	...
	log.Info("Light TTC protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light TTC peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light TTC handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light TTC peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light TTC message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light TTC message arrived", "code", msg.Code, "bytes", msg.Size)
